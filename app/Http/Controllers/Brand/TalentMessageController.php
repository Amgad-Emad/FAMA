 ...
 * a contract, so this opens the brand↔talent thread:
 *  - Guest / non-brand → brand authentication, returning here afterwards.
 *  - Authenticated brand → the most recent contract with this talent, or a fresh one
 *    started on the default flow, then straight into the contract room (where the free
...
class TalentMessageController extends Controller
{
    ...
    /**
     * Open (or lazily start) the brand↔talent contract for $talent and land in its room.
     */
    public function __invoke(Request $request, Talent $talent): RedirectResponse
    {
        ...
        // Reuse the latest existing contract with this talent, else start one so there is
        // always a thread to message in.
        $contract = $brand->contracts()->where('talent_id', $talent->id)->latest()->first();
        ...
        return redirect()->route('brand.contracts.show', $contract);
    }
}
