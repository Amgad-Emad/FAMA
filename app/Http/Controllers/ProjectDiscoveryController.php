...
/**
 * Public project browsing (talent-facing) — the "opportunities" board: open,
 * public projects from published brands that a talent can browse and message
...
class ProjectDiscoveryController extends Controller
{
    ...
    public function feed(Request $request): JsonResponse
    {
        ...
        $paginator = BrandProject::query()
            ->where('is_public', true)
            ->whereIn('status', ['open', 'in_progress'])
            ->whereHas('brand', fn ($query) => $query->where('is_published', true))
            ->when($q !== '', fn ($query) => $query->where('title', 'like', "%{$q}%"))
            ->when($types !== [], fn ($query) => $query->whereHas('talentType', fn ($tt) => $tt->whereIn('talent_types.slug', $types)))
            ...
            ->with(['brand.media', 'talentType', 'media'])
            ->latest()
            ->paginate(12)
            ->withQueryString();

        return response()->paginated($paginator, PublicProjectCardResource::collection($paginator->getCollection()));
    }
}
