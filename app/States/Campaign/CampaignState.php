...
/**
 * Campaign lifecycle (brand-spec §5). draft → open → in_progress → completed;
 * cancellable from any active state. `is_public` toggles listed ⇄ private
 * independently of status; soft-delete removes it. A completed campaign becomes
...
abstract class CampaignState extends State
{
    ...
    /**
     * True while the campaign is live (not completed/cancelled).
     */
