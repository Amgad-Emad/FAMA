<x-admin-layout :title="__('Deal intervention')">
    <div x-data="adminDeal({{ $deal->id }})" x-cloak class="grid gap-8 lg:grid-cols-3">
        <div class="space-y-4 lg:col-span-2">
            <a href="{{ route('admin.deals') }}" class="text-xs text-muted hover:text-ink">← {{ __('All deals') }}</a>
            ...
            <template x-if="deal">
                <div class="space-y-4">
                    <x-ui.card>
                        ...
                        <div class="mt-1 text-sm text-muted"><span x-text="deal.brand?.name"></span> ↔ <span x-text="deal.talent?.display_name"></span></div>
                        <span class="mt-3 inline-flex rounded-pill bg-elevated px-2 py-0.5 text-[10px] font-medium text-muted" x-text="deal.status.replaceAll('_',' ')"></span>
                    </x-ui.card>
                    ...
                    <ol class="rounded-lg border border-line bg-surface p-3">
                        <template x-for="step in steps" :key="step.id">
                            <li class="flex items-center gap-3 rounded-md px-2 py-2" :class="step.is_current ? 'bg-accent-weak' : ''">
                                ...
                                <span class="font-mono text-[10px] uppercase text-subtle" x-text="step.actor"></span>
                            </li>
                        </template>
                    </ol>
                    ...
                    <x-ui.card class="space-y-3">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            <x-ui.button size="sm" variant="outline" x-on:click="act('cancel', { reason: note })" x-bind:disabled="acting">{{ __('Cancel deal') }}</x-ui.button>
                        </div>
                    </x-ui.card>
                </div>
            </template>
