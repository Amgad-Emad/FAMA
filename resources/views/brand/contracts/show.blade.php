<x-brand-layout :title="__('Contract room')">
    <div x-data="brandContractRoom({{ $contract->id }}, { brief: @js(__('Brief')), startDate: @js(__('Start date')), endDate: @js(__('End date')), initiatedBy: @js(__('Initiated by')) })" x-cloak class="mx-auto max-w-6xl space-y-6">

        {{-- Back --}}
        <a href="{{ route('brand.contracts') }}" class="inline-flex items-center gap-1.5 text-sm text-muted transition hover:text-ink">
        ...
        <template x-if="contract">
            <div class="space-y-6">
                ...
                <div class="rounded-xl border border-line bg-surface p-6 shadow-e1">
                    <div class="flex flex-wrap items-start justify-between gap-4">
                        <div class="min-w-0">
                            ...
                            <h1 class="mt-0.5 font-display text-2xl leading-tight text-ink" x-text="contract.title"></h1>
                            <template x-if="contract.project">
                                <a :href="`/brand/projects/${contract.project.slug}`"
                                   class="mt-1 inline-flex items-center gap-1.5 text-xs font-medium text-accent-ink hover:underline">
                                    ...
                                    <span x-text="contract.project.title"></span>
                                </a>
                            </template>
                        </div>
                        <span class="shrink-0 rounded-pill px-3 py-1 text-[11px] font-semibold uppercase tracking-wide"
                              :class="contract.is_brand_turn ? 'bg-accent text-on-accent' : (contract.status === 'completed' ? 'bg-success-weak text-success' : 'border border-line-strong bg-elevated text-muted')"
                    ...
                    <div class="mt-5 flex flex-wrap items-center justify-between gap-4 border-t border-line pt-4">
                        <div class="flex items-center gap-3">
                            <span class="grid h-10 w-10 shrink-0 place-items-center overflow-hidden rounded-pill border border-line-strong bg-elevated font-display text-sm text-accent-ink">
                                <template x-if="contract.talent?.avatar_url"><img :src="contract.talent.avatar_url" alt="" class="h-full w-full object-cover"></template>
                                <template x-if="!contract.talent?.avatar_url"><span x-text="(contract.talent?.display_name || '·').slice(0,1)"></span></template>
                            </span>
                            <div>
                                <div class="font-mono text-[10px] uppercase tracking-wider text-subtle">{{ __('Talent') }}</div>
                                <a :href="`/${contract.talent?.slug}`" class="text-sm font-medium text-ink hover:text-accent-ink" x-text="contract.talent?.display_name || '—'"></a>
                            </div>
                        </div>
                        <template x-if="contract.agreed_amount">
                            <div class="text-end">
                                <div class="font-mono text-[10px] uppercase tracking-wider text-subtle">{{ __('Agreed') }}</div>
                                <div class="font-display text-xl text-ink"><span x-text="Number(contract.agreed_amount).toLocaleString()"></span> <span class="text-sm text-muted" x-text="contract.currency"></span></div>
                            </div>
                        </template>
                    </div>
                </div>
                ...
                <div class="grid gap-6 lg:grid-cols-3">
                    ...
                    {{-- Wide: contract details + the message timeline (the centre of the room) --}}
                    <div class="space-y-6 lg:col-span-2">
                        {{-- Contract details --}}
                        <template x-if="detailRows.length">
                            <div class="rounded-xl border border-line bg-surface p-6 shadow-e1">
                                <div class="mb-4 font-mono text-[10px] uppercase tracking-wider text-subtle">{{ __('Contract details') }}</div>
                                            ...
                                            <div class="flex" :class="m.sender_role === 'brand' ? 'justify-end' : 'justify-start'">
                                                <div class="max-w-[78%] rounded-2xl px-3.5 py-2 text-sm shadow-e1"
                                                    ...
                                                    <template x-if="m.attachments && m.attachments.length">
                                                        <div class="mt-2 flex flex-col gap-1.5 border-t pt-2" :class="m.sender_role === 'brand' ? 'border-white/25' : 'border-line'">
                        ...
                        <div class="rounded-xl border border-line bg-surface p-5 shadow-e1">
                            <template x-if="!currentStep">
                                <div class="text-center">
                                    ...
                                    <p class="mt-2 text-sm font-medium text-ink">{{ __('This contract is finished.') }}</p>
                                </div>
                            </template>
