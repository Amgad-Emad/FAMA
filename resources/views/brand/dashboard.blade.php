    ...
    <div class="space-y-8">
        ...
        <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
            @foreach ([
                ...
                ['label' => __('Active deals'), 'value' => $stats['active_deals']],
                ['label' => __('Campaigns'), 'value' => $stats['campaigns']],
        ...
        <section>
            <div class="mb-3 flex items-center justify-between">
                <h3 class="font-display text-lg">{{ __('Active deals') }}</h3>
                <a href="{{ route('brand.deals') }}" class="text-xs text-muted hover:text-ink">{{ __('View all') }}</a>
            </div>
            @forelse ($activeDeals as $deal)
                <a href="{{ route('brand.deals.show', $deal) }}" class="mb-2 flex items-center justify-between rounded-lg border border-line bg-surface px-4 py-3 hover:border-line-strong">
                    <div>
                        <div class="text-sm font-medium">{{ $deal->title }}</div>
                        <div class="text-xs text-muted">{{ $deal->talent?->display_name }} · {{ $deal->reference }}</div>
                    </div>
                    @if ((string) $deal->status === 'awaiting_brand')
                        <span class="rounded-pill bg-accent px-2 py-0.5 text-xs text-on-primary">{{ __('Your turn') }}</span>
                    @else
                        <span class="rounded-pill bg-elevated px-2 py-0.5 text-xs text-muted">{{ __(ucfirst(str_replace('_',' ', (string) $deal->status))) }}</span>
                    @endif
                </a>
            @empty
                <p class="rounded-lg border border-dashed border-line px-4 py-6 text-center text-sm text-muted">{{ __('No active deals yet.') }}</p>
            @endforelse
        </section>
        ...
        <section>
            <div class="mb-3 flex items-center justify-between">
                <h3 class="font-display text-lg">{{ __('Recent campaigns') }}</h3>
                <a href="{{ route('brand.campaigns') }}" class="text-xs text-muted hover:text-ink">{{ __('Manage') }}</a>
            </div>
            @forelse ($recentCampaigns as $campaign)
                <a href="{{ route('brand.campaigns.show', $campaign) }}" class="mb-2 flex items-center justify-between rounded-lg border border-line bg-surface px-4 py-3 hover:border-line-strong">
            ...
            @empty
                <p class="rounded-lg border border-dashed border-line px-4 py-6 text-center text-sm text-muted">{{ __('No campaigns yet.') }}</p>
            @endforelse
        </section>
    </div>
</x-brand-layout>
