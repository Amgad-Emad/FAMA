<x-brand-layout :title="__('Deal room')">
    <div x-data="brandDealRoom({{ $deal->id }})" x-cloak class="grid gap-8 lg:grid-cols-3">
            ...
            <template x-if="deal">
                <div class="space-y-4">
                    ...
                    <div class="rounded-lg border border-line bg-surface p-5">
                        ...
                        <div class="mt-1 text-sm text-muted" x-text="deal.talent?.display_name"></div>
                        <div class="mt-3 flex flex-wrap items-center gap-2">
                            <span class="rounded-pill px-2 py-0.5 text-[10px] font-medium"
                                  :class="deal.is_brand_turn ? 'bg-accent text-on-primary' : 'border border-line bg-surface text-muted'"
                            ...
                            <div class="space-y-4">
                                ...
                                <template x-if="currentStep.step_type === 'approval'">
                                    <div class="space-y-2">
                                        ...
                                        <div class="flex gap-2">
                                            <x-ui.button variant="accent" x-on:click="approve()" x-bind:disabled="acting">{{ __('Accept') }}</x-ui.button>
                                ...
                                <template x-if="currentStep.step_type === 'upload'">
                                    <div class="space-y-2">
                                        ...
                                        <x-ui.button variant="accent" x-on:click="deliver()" x-bind:disabled="acting">{{ __('Upload') }}</x-ui.button>
                                    </div>
                                </template>

                                {{-- payment --}}
                                <template x-if="currentStep.step_type === 'payment'">
                                    <x-ui.button variant="accent" x-on:click="pay()" x-bind:disabled="acting">{{ __('Pay now') }}</x-ui.button>
                                </template>
        ...
        <div class="space-y-4">
                            ...
                            <template x-if="!m.is_system">
                                <div class="flex" :class="m.sender_role === 'brand' ? 'justify-end' : 'justify-start'">
                                    <div class="max-w-[80%] rounded-lg px-3 py-2 text-sm"
                                         :class="m.sender_role === 'brand' ? 'bg-accent text-on-accent' : 'border border-line bg-elevated text-ink'"
                                         x-text="m.body"></div>
                                </div>
                            </template>
                        </div>
            ...
            <a href="{{ route('brand.deals') }}" class="block text-center text-sm text-muted hover:text-ink">← {{ __('All deals') }}</a>
        </div>
    </div>
</x-brand-layout>
