...
<x-brand-layout :title="$campaign->title">
    <div x-data="brandProject({{ $campaign->id }}, @js($initial))" class="space-y-6">
        <a href="{{ route('brand.projects') }}" class="inline-flex items-center gap-1.5 text-xs text-muted transition hover:text-ink">
            <span class="rtl:rotate-180">←</span> {{ __('All projects') }}
        </a>
                ...
                <section class="overflow-hidden rounded-2xl border border-line bg-surface shadow-e1">
                        ...
                        <div class="flex flex-wrap items-start justify-between gap-4">
                            <div class="min-w-0">
                                ...
                                <p class="mt-1.5 flex items-center gap-2 text-sm text-muted">
                                    <span><span x-text="campaign.contracts_count || 0"></span> {{ __('contracts') }}</span>
                    ...
                    <div x-show="editing" x-cloak class="border-t border-line bg-elevated/40 p-6 sm:p-7">
                        <div class="mb-6 flex items-start gap-3">
                            ...
                            <div>
                                <h3 class="font-display text-lg text-ink">{{ __('Edit project') }}</h3>
                                <p class="text-xs text-muted">{{ __('Update the project details, budget, and roles.') }}</p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-5">
                            ...
                            <div class="rounded-xl border border-line bg-surface p-5">
                                ...
                                <div class="grid gap-4 sm:grid-cols-2">
                                    ...
                                    {{-- Brief (translatable) --}}
                                    <div class="sm:col-span-2"><x-input-label :value="__('Brief (EN)')" /><textarea rows="2" class="mt-1 block w-full rounded-md border-line bg-bg text-sm" x-model="form.description.en"></textarea></div>
                                    <div class="sm:col-span-2"><x-input-label :value="__('Brief (AR)')" /><textarea rows="2" dir="rtl" class="mt-1 block w-full rounded-md border-line bg-bg text-sm" x-model="form.description.ar"></textarea></div>
                                </div>
                            </div>
                            ...
                            <div class="rounded-xl border border-line bg-surface p-5">
                                ...
                                {{-- Budget visibility — private by default; only you see a private budget. --}}
                            ...
                            {{-- Role — one role, one position (a single discipline). --}}
                            <div class="rounded-xl border border-line bg-surface p-5">
                                <div class="mb-3 font-mono text-[10px] uppercase tracking-[0.16em] text-subtle">{{ __('Role') }}</div>
                                <select class="block w-full rounded-md border-line bg-bg text-sm" x-model.number="form.talent_type_id">
                                ...
                                <p class="mt-2 text-xs text-muted">{{ __('A project seeks one talent in one role.') }}</p>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-2 overflow-hidden rounded-2xl border border-line sm:grid-cols-4" style="gap:1px;background:var(--line)">
                            <div class="bg-surface px-5 py-4">
                                <div class="flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-wider text-subtle">
                                    {{ __('Budget') }}
                                    {{-- Owner always sees the budget; the tag shows whether talent can too. --}}
                                    <span class="rounded-pill px-1.5 py-0.5 text-[9px] font-semibold normal-case tracking-normal"
                                          :class="campaign.budget_is_public ? 'bg-accent-weak text-accent-ink' : 'bg-elevated text-muted'"
                                          x-text="campaign.budget_is_public ? '{{ __('Public') }}' : '{{ __('Private') }}'"></span>
                                </div>
                            ...
                            <div class="bg-surface px-5 py-4">
                                <div class="font-mono text-[10px] uppercase tracking-wider text-subtle">{{ __('Contracts') }}</div>
                                <div class="mt-1 font-display text-lg text-ink" x-text="campaign.contracts_count || 0"></div>
                            </div>
                            <div class="bg-surface px-5 py-4">
                                <div class="font-mono text-[10px] uppercase tracking-wider text-subtle">{{ __('Role') }}</div>
                                <div class="mt-1 truncate font-display text-lg text-ink" x-text="campaign.role?.name || '—'"></div>
                            </div>
                            <div class="bg-surface px-5 py-4">
                        ...
                        <div class="grid gap-6 lg:grid-cols-[1fr_320px]">
                            ...
                            <div class="flex min-w-0 flex-col gap-6">
                                {{-- Brief --}}
                                <section x-show="campaign.description && (campaign.description.en || campaign.description.ar)" x-cloak class="rounded-2xl border border-line bg-surface p-6">
                                    <h3 class="mb-3 font-display text-lg text-ink">{{ __('Brief') }}</h3>
                                    <p class="whitespace-pre-line text-sm leading-relaxed text-muted" x-text="t(campaign.description)"></p>
                                </section>

                                {{-- Role — one role, one position. --}}
                                <section class="rounded-2xl border border-line bg-surface p-6">
                                    <h3 class="mb-4 font-display text-lg text-ink">{{ __('Role sought') }}</h3>
                                    <template x-if="campaign.role">
                                        <div class="flex items-center justify-between gap-3 rounded-xl border border-line bg-elevated px-4 py-3">
                                            <span class="min-w-0 truncate font-medium text-ink" x-text="campaign.role.name"></span>
                                            <span class="shrink-0 rounded-pill bg-accent-weak px-2.5 py-1 font-mono text-[11px] text-accent-ink">× 1</span>
                                        </div>
                                    </template>
                                    <template x-if="!campaign.role"><p class="text-sm text-muted">{{ __('No role defined.') }}</p></template>
                                </section>
                                ...
                                <section class="rounded-2xl border border-line bg-surface p-6">
                                    <h3 class="mb-4 font-display text-lg text-ink">{{ __('Contracts under this project') }}</h3>
                                    <div class="space-y-2">
                                        <template x-for="contract in contracts" :key="contract.id">
                                            <a :href="`/brand/contracts/${contract.id}`" class="flex items-center justify-between gap-3 rounded-xl border border-line px-4 py-3 transition hover:border-line-strong">
                                                <div class="min-w-0">
                                                    <div class="truncate text-sm font-medium text-ink" x-text="contract.title"></div>
                                                    <div class="text-xs text-muted" x-text="contract.talent?.display_name || contract.reference"></div>
                                                </div>
                                                <span class="shrink-0 rounded-pill bg-elevated px-2.5 py-1 text-[11px] capitalize text-muted" x-text="contract.status.replaceAll('_', ' ')"></span>
                                            </a>
                                        </template>
                                        <template x-if="!contracts.length">
                                            <p class="rounded-xl border border-dashed border-line py-8 text-center text-sm text-muted">{{ __('No contracts under this project yet.') }}</p>
                                        </template>
                                    </div>
                                </section>
                            </div>
                            ...
                            <aside class="flex flex-col gap-4 lg:sticky lg:top-20 lg:self-start">
                                <div class="rounded-2xl border border-line bg-surface p-5">
                                    ...
                                    @if ($brandUser?->is_published && $brandUser?->slug)
                                        <a x-show="campaign.is_public" x-cloak :href="`/brands/{{ $brandUser->slug }}/projects/${campaign.slug}`" target="_blank" rel="noopener"
