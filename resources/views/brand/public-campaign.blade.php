@php
    ...
    $meta = collect([
        ['k' => __('Budget'), 'v' => $budget->isNotEmpty() ? $budget->map(fn ($v) => number_format((float) $v))->implode(' – ').' '.$campaign->currency : null],
        ['k' => __('Dates'), 'v' => $dates ?: null],
        ['k' => __('Location'), 'v' => $location ?: null],
        ['k' => __('Roles'), 'v' => $campaign->talentTypes->isNotEmpty() ? $campaign->talentTypes->sum(fn ($t) => (int) $t->pivot->quantity) : null],
            ...
            <div class="flex min-w-0 flex-col gap-6">
                ...
                @if ($campaign->talentTypes->isNotEmpty())
                    <section>
                        ...
                        <h2 class="mt-1 font-display text-2xl tracking-tight text-ink sm:text-3xl">{{ __('Roles sought') }}</h2>
                        <div class="mt-5 flex flex-col gap-3">
                            @foreach ($campaign->talentTypes as $type)
                                <div class="flex flex-wrap items-center justify-between gap-4 rounded-md border border-line bg-surface px-5 py-4">
                                    <div class="flex items-center gap-3">
                                        <span class="font-display text-xl text-ink">{{ $type->getTranslation('name', app()->getLocale()) }}</span>
                                        <span class="rounded-pill bg-accent-weak px-2.5 py-1 font-mono text-[11px] text-accent-ink">× {{ (int) $type->pivot->quantity }}</span>
                                    </div>
                                    <a href="{{ url('/discover') }}?filter[type]={{ $type->slug }}" class="font-mono text-xs text-muted transition hover:text-accent-ink">{{ __('Find talent') }} →</a>
                                </div>
                            @endforeach
                        </div>
                    </section>
