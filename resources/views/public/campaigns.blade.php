<x-public-layout :title="__('Opportunities')">
    <div class="mx-auto max-w-6xl px-4 py-8 sm:px-6"
        ...
        <header class="mb-6">
            ...
            <h1 class="mt-1 font-display text-4xl text-ink">{{ __('Open campaigns') }}</h1>
        </header>
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        <div class="relative w-full sm:w-52">
                            <label for="campaigns-q" class="sr-only">{{ __('Search campaigns…') }}</label>
                            ...
                            <input id="campaigns-q" x-model="filters.q" @input.debounce.400ms="load(1)" type="text" placeholder="{{ __('Search campaigns…') }}"
        ...
        <div class="mb-4">
            <p class="text-sm text-muted" aria-live="polite">
                ...
                <span x-show="!loading" x-cloak><span class="font-semibold text-ink" x-text="resultTotal"></span> {{ __('campaigns looking for talent') }}</span>
            </p>
        </div>
        ...
        <div x-show="!loading" x-cloak class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            <template x-for="c in campaigns" :key="c.id">
                <div class="group flex flex-col overflow-hidden rounded-xl border border-line bg-surface shadow-e1 transition hover:-translate-y-0.5 hover:shadow-e3">
                    <a :href="`/brands/${c.brand?.slug}/campaigns/${c.slug}`" class="relative block aspect-[16/10] bg-elevated bg-cover bg-center"
                    ...
                    <div class="flex min-w-0 flex-1 flex-col gap-3 p-5">
                        <div class="min-w-0">
                            ...
                            <a :href="`/brands/${c.brand?.slug}/campaigns/${c.slug}`" class="mt-1.5 block truncate font-display text-lg text-ink transition group-hover:text-accent-ink" x-text="c.title"></a>
                        </div>
                        <div class="flex flex-wrap gap-1.5" x-show="c.roles && c.roles.length" x-cloak>
                            <template x-for="role in (c.roles || [])" :key="role.name">
                                <span class="rounded-pill bg-elevated px-2 py-0.5 text-[11px] text-muted"><span x-text="role.name"></span> × <span x-text="role.quantity"></span></span>
                            </template>
                        </div>
                        <div class="mt-auto flex items-center justify-between gap-2 pt-2">
                            <span class="min-w-0 truncate font-mono text-[11px] text-subtle" x-show="c.location" x-cloak x-text="c.location"></span>
                            <a :href="`/brands/${c.brand?.slug}/message?campaign=${c.id}`" class="shrink-0 rounded-pill bg-accent px-3 py-1.5 text-xs font-medium text-on-accent transition hover:opacity-90">{{ __('Message brand') }}</a>
                        </div>
                    </div>
                </div>
            </template>
        ...
        <div x-show="!loading && campaigns.length === 0" x-cloak class="rounded-xl border border-dashed border-line py-16 text-center">
            <p class="text-sm text-subtle">{{ __('No open campaigns right now.') }}</p>
