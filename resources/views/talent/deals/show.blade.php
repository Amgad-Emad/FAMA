<x-talent-layout :title="__('Deal room')">
    <div x-data="dealRoom({{ $deal->id }})" x-cloak class="space-y-6">

        {{-- Header (top, full width): back link + reference, title, counterparty, status, amount --}}
        <div>
            <a href="{{ route('talent.deals') }}" class="mb-3 inline-flex items-center gap-1 text-sm text-muted hover:text-ink">← {{ __('All deals') }}</a>
