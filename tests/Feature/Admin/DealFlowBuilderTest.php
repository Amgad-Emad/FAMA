...
beforeEach(function () {
    ...
    $this->builder = app(DealFlowBuilderService::class);
});
...
it('editing a flow only affects future deals (existing deal keeps its snapshot)', function () {
    ...
    expect($deal->steps()->where('key', 'brief')->first()->name)->toBe('Original Brief');
});
