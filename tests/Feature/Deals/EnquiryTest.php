...
it('renders the booking form and stores a new enquiry', function () {
    $talent = Talent::factory()->create(['slug' => 'jane', 'availability_status' => 'available']);
...
it('rejects an enquiry when the talent is unavailable (422)', function () {
    Talent::factory()->create(['slug' => 'jane', 'availability_status' => 'unavailable']);

    $this->postJson(route('talent.enquire.store', ['slug' => 'jane']), [
        'contact_name' => 'Acme', 'contact_email' => 'hi@acme.com', 'brief' => 'x',
    ])->assertStatus(422);

    expect(DealEnquiry::count())->toBe(0);
});

it('validates the enquiry (missing name & brief → 422)', function () {
    Talent::factory()->create(['slug' => 'jane', 'availability_status' => 'available']);
